...
# 기존 서비스 삭제
log "en-service undeploy"
docker rm -f en-service

# 기존 en-service 이미지를 삭제하고 새로 빌드
log "en-service image remove and build."
docker rmi en-service:latest || true
docker build -t en-service:latest .

# Docker로 en-service 서비스 실행
log "Execute en-service..."
docker run -d \
  --name en-service \
